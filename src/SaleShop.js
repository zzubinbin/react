...
class SaleShop extends React.Component {
  ...
  componentWillMount() {
    console.log("111组件将要渲染");
  }
  componentDidMount() {
    console.log("111组件渲染完成");
    this.getJsonData();
  }
  componentWillReceiveProps(nextProps, nextContext) {
    console.log("111将要接收一个新的props");
  }
  componentWillUpdate(nextProps, nextState) {
    //在组件接收到新的props或者state但还没有render时被调用
    console.log('111在组件接收到新的props或者state但还没有render时被调用');
  }
  componentDidUpdate(prevProps, prevState) {
    //在组件完成更新后立即调用。在初始化时不会被调用。
    console.log('111在组件完成更新后立即调用。在初始化时不会被调用。')
  }
  componentWillUnmount() {
    //在组件从 DOM 中移除之前立刻被调用。
    console.log('111在组件从 DOM 中移除之前立刻被调用。')
  }
  render() {
    return (
      <React.Fragment>
